...
olcSprite::olcSprite(const std::wstring& sFile) {
	if (!Load(sFile))
		Create(8, 8);
}
...
wchar_t olcSprite::GetGlyph(int x, int y) const
{
	if (x <0 || x >= nWidth || y < 0 || y >= nHeight)
		return L' ';
	else
...
short olcSprite::GetColour(int x, int y) const
{
	if (x <0 || x >= nWidth || y < 0 || y >= nHeight)
		return FG_BLACK;
	else
...
wchar_t olcSprite::SampleGlyph(float x, float y) const
{
...
short olcSprite::SampleColour(float x, float y) const
{
	int sx = (int)(x * (float)nWidth);
	int sy = (int)(y * (float)nHeight - 1.0f);
	if (sx <0 || sx >= nWidth || sy < 0 || sy >= nHeight)
...
bool olcSprite::Save(const std::wstring& sFile) const
{
	FILE *f = nullptr;
...
bool olcSprite::Load(const std::wstring& sFile)
{
	delete[] m_Glyphs;
	delete[] m_Colours;
	nWidth = 0;
...
#pragma region olcConsoleGameEngine
...
void olcConsoleGameEngine::DrawString(int x, int y, const std::wstring& c, short col)
{
...
void olcConsoleGameEngine::DrawStringAlpha(int x, int y, const std::wstring& c, short col)
{
	for (size_t i = 0; i < c.size(); i++)
	{
		if (c[i] != L' ')
...
void olcConsoleGameEngine::DrawWireFrameModel(const std::vector<std::pair<float, float>> &vecModelCoordinates, float x, float y, float r, float s, short col)
{
